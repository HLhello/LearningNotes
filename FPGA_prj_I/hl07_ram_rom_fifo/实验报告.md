...
4. FIFO的使用
   ...
   4. 实现 FIFO 的方法
      ...
      1. 第一种为用户根据需求自己编写 FIFO 逻辑，当对于 FIFO 的功能有特殊需求时，可以使用此种方式实现
         ...
         2. 对于fifo的深度——分析数据重载和轻载时的传输任务
            1. 写时钟频率：wclk；读时钟频率：rclk
            2. 写的时候每b个时钟会有a个数据写入fifo；读的时候每y个时钟会有x个数据读出fifo
            3. $burst_{length}$表示这段时间写入的数据量；$burst_{length}/t_{wclk}$表示burst持续的时间
            4. $rclk * x/y$表示读数据的实际速度
            5. $depth = burst_{length} - burst_{length}/t_{wclk} * rclk * x/y$
            6. 两者差值就是fifo中数据的残留量，也就是fifo的最小深度
            7. 例：数据传输任务为每100个写时钟写入80个数据，也就是有20个写时钟是空闲的，背靠背（最坏情况）也就是在160个写时钟内写入160个数据，这是传输任务要求的最长的写如数据成为burst长度
               1. burst持续的时间：$burst_{cycle} * t_{wclk} = burst_{cycle}/f_{wclk} = 160/100$
               2. $data_{numread} = Time/t_r = Time * f_r = 160/100*80$
               3. $depth = burst - data_{numread} = 160-160/100*80 = 32$
