...
支持AXI4总线的从机大致可以分为两种类型，一种是内存类型的从机（memory slave），一种是外设类型的从机（peripheral slave）。

但是说到底，这两种类型的从机就是一个地址空间下的寄存器阵列，本质上没有区别。区别只在于内存类型的从机工作的内容就是将写入的数据存储，再按照地址索引读出，而外设类型的从机寄存器与外设本身的动作有关。
