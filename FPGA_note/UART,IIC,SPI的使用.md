...
UART，SPI，IIC的详解，UART，SPI，IIC是经常用到的几个数据传输标准，下面分别总结一下：

 

如何验证RTL级代码是正确的

编写仿真文件，使用modelsim进行仿真，如果需要，对模块使用matlab建模，分析输入输出是否正确。

 

 

UART（通用异步接收发送器）：也叫串口，常用于调试。

主机和从机主要用两根线来通信，TX用于发送数据，RX用于接受数据（收发不是一根线，所以是全双工方式）。
...
SPI的连接一般有四根：时钟，发送，接收和片选。需要与那个芯片通信，拉低该芯片的片选，该总线大量用于与EEPROM，ADC，和显示驱动器之类的慢速外设器件通信。

IIC——Inter Integrated Circuit

IIC一般包括两根线：一个时钟线SCL和一个数据线SDA。只有一根数据线，所以是半双工通信。接线不难，而且两根线上也可以挂很多设备（每个设备的IIC地址不同）。与那个芯片进行通信，先把地址打入到总线上的芯片中，检测到地址芯片从sda发回应答信号。

​    IIC通信过程IIC 通信过程大概如下。首先，主设备发一个START信号，这个信号就像对所有其它设备喊：请大家注意！然后其它设备开始监听总线以准备接收数据。接着，主设备发送一个7位设备地址加一位的读写操作的数据帧。当所设备接收数据后，比对地址自己是否目标设备。如果比对不符，设备进入等待状态，等待STOP信号的来临；如果比对相符，设备会发送一个应答信号——ACKNOWLEDGE作回应。
